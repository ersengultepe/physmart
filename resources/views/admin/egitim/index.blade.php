...
@section('meta')
    <title>SDS Danışmanlık | Eğitimler</title>
@endsection
...
@section('content')
    <section class="panel">
        <header class="panel-heading">
            ...
            <h2 class="panel-title">Eğitimler</h2>
        </header>
        <div class="panel-body">
            ...
            <table class="table table-bordered table-striped mb-none" id="datatable-tabletools" data-swf-path="assets/vendor/jquery-datatables/extras/TableTools/swf/copy_csv_xls_pdf.swf">
                ...
                @foreach($egitimler as $egitim)
                    <tr class="">
                        <td><img style="height: 25%" src="{{ asset($egitim->image) }}" alt="{{ $egitim->title }}" ></td>

                        <td><a href="{{ route('egitim.show', ['id'=>$egitim->id]) }}">{!! $egitim->title !!}</a></td>

                        <td >{{ $egitim->getAuthor->name.' '.$egitim->getAuthor->surname }}</td>

                        <td >{{ App\Http\Controllers\Helper::getTime($egitim->created_At) }}</td>

                        <td >
                            <form method="get" action="{{ route('egitim.edit', ['id' => $egitim->id]) }}">
                                {!! Form::hidden('_method', 'PATCH') !!}
                                <input type="submit" value="Düzenle" class="btn btn-warning">
                            </form>
                        </td>
                        <td>
                            {!! Form::open(['url' => route('egitim.destroy', ['id' => $egitim->id])]) !!}
