...
@section('meta')
    <title>SDS Danışmanlık | Hizmetler</title>
@endsection
        ...
        <div class="panel-body">
            ...
            <table class="table table-bordered table-striped mb-none" id="datatable-tabletools" data-swf-path="assets/vendor/jquery-datatables/extras/TableTools/swf/copy_csv_xls_pdf.swf">
                ...
                @foreach($hizmetler as $hizmet)
                    <tr class="">
                        <td><img style="height: 25%" src="{{ asset($hizmet->image) }}" alt="{{ $hizmet->title }}" ></td>

                        <td><a href="{{ route('hizmet.show', ['id'=>$hizmet->id]) }}">{!! $hizmet->title !!}</a></td>

                        <td >{{ $hizmet->getAuthor->name.' '.$hizmet->getAuthor->surname }}</td>

                        <td >{{ App\Http\Controllers\Helper::getTime($hizmet->created_At) }}</td>

                        <td >
                            <form method="get" action="{{ route('hizmet.edit', ['id' => $hizmet->id]) }}">
                                {!! Form::hidden('_method', 'PATCH') !!}
                                <input type="submit" value="Düzenle" class="btn btn-warning">
                            </form>
                        </td>
                        <td>
                            {!! Form::open(['url' => route('hizmet.destroy', ['id' => $hizmet->id])]) !!}
