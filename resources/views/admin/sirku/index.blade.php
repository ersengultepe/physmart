...
@section('meta')
    <title>SDS Danışmanlık | Sirküler</title>
@endsection
...
@section('content')
    <section class="panel">
        <header class="panel-heading">
            ...
            <h2 class="panel-title">Sirküleriniz</h2>
        </header>
        <div class="panel-body">
            ...
            <table class="table table-bordered table-striped mb-none" id="datatable-tabletools" data-swf-path="assets/vendor/jquery-datatables/extras/TableTools/swf/copy_csv_xls_pdf.swf">
                ...
                <tbody>
                @foreach($sirkuler as $sirku)
                    <tr class="">
                        <td><a href="{{ route('sirku.show', ['id'=>$sirku->id]) }}">{!! $sirku->title !!}</a></td>

                        <td>{{ $sirku->getCategory->title }}</td>

                        <td >{{ $sirku->getAuthor->name.' '.$sirku->getAuthor->surname }}</td>

                        <td >{{ App\Http\Controllers\Helper::getTime($sirku->created_At) }}</td>

                        <td >
                            <form method="get" action="{{ route('sirku.edit', ['id' => $sirku->id]) }}">
                                {!! Form::hidden('_method', 'PATCH') !!}
                                <input type="submit" value="Düzenle" class="btn btn-warning">
                            </form>
                        </td>
                        <td>
                            {!! Form::open(['url' => route('sirku.destroy', ['id' => $sirku->id])]) !!}
