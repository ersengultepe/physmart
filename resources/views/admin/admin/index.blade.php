...
@section('content')
    <section class="panel">
        ...
        <div class="panel-body">
            <span>
                <a href="{{ route('admin.create') }}" class="btn btn-defaut">Kullanıcı Ekle</a>
            </span>
            ...
            <table class="table table-bordered table-striped mb-none" id="datatable-tabletools" data-swf-path="assets/vendor/jquery-datatables/extras/TableTools/swf/copy_csv_xls_pdf.swf">
                ...
                <tr>
                    <th>Foto</th>
                    <th>Ad Soyad</th>
                    <th width="15%">Şifre Güncelle</th>
                    <th width="15%">Profil Güncelle</th>
                    <th width="5%">Sil</th>
                </tr>
                </thead>
                <tbody>
                @foreach($users as $user)
                    <tr class="">
                        <td><img style="height: 25%" src="{{ asset($user->image) }}" ></td>
                        ...
                        <td >
                            <form method="get" action="{{ route('admin.edit', ['id' => $user->id]) }}">
                                ...
                                {!! Form::hidden('tur', 'pass') !!}
                                <input type="submit" value="Şifre Güncelle" class="btn btn-info">
                            </form>
                        </td>
                        ...
                        <td >
                            <form method="get" action="{{ route('admin.edit', ['id' => $user->id]) }}">
                                {!! Form::hidden('_method', 'PATCH') !!}
                                {!! Form::hidden('tur', 'profile') !!}
                                <input type="submit" value="Profil Güncelle" class="btn btn-warning">
                            </form>
                        </td>

                        <td>
                            {!! Form::open(['url' => route('admin.destroy', ['id' => $user->id])]) !!}
