...
@section('content')
    <section class="panel">
        <header class="panel-heading">
            ...
            <h2 class="panel-title">Haberler</h2>
        </header>
        <div class="panel-body">
            ...
            <table class="table table-bordered table-striped mb-none" id="datatable-tabletools" data-swf-path="assets/vendor/jquery-datatables/extras/TableTools/swf/copy_csv_xls_pdf.swf">
                ...
                @foreach($haberler as $haber)
                    <tr class="">
                        <td>{{ $haber->id }}</td>
                        <td><img style="height: 25%" src="{{ asset($haber->image) }}" alt="{{ $haber->title }}" ></td>

                        <td><a href="{{ route('haber.show', ['id'=>$haber->id]) }}">{!! $haber->title !!}</a></td>

                        <td>{{ $haber->getCategory->title }}</td>

                        <td>{{ $haber->getAuthor->name.' '.$haber->getAuthor->surname }}</td>

                        <td>{{ App\Http\Controllers\Helper::getTime($haber->created_at) }}</td>

                        <td>
                            <form method="get" action="{{ route('haber.edit', ['id' => $haber->id]) }}">
                                {!! Form::hidden('_method', 'PATCH') !!}
                                <input type="submit" value="Düzenle" class="btn btn-warning">
                            </form>
                        </td>
                        <td>
                            {!! Form::open(['url' => route('haber.destroy', ['id' => $haber->id])]) !!}
                        ...
                        <td>
                            <a href="{{ url('manage/haber/tosirku/'.$haber->id) }}">
